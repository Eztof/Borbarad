...
if (!el) return;
el.innerHTML = user ? `
<span class="small">Angemeldet: ${user.email}</span>
...
export function dateBadge(av){
return `<span class="tag">${formatAvDate(av)}</span>`;
}


export function section(title, actionsHtml=''){
return `<div class="toolbar"><h2 style="margin:6px 0">${title}</h2><div>${actionsHtml}</div></div>`;
}
...
export function avDateInputs(idPrefix, value){
return formRow('Datum (Aventurisch)', `<div class="card">${datePickerAv(idPrefix, value)}</div>`);
}